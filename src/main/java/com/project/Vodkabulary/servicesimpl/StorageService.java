...
import com.project.Vodkabulary.services.IStorageService;

@Service
public class StorageService implements IStorageService {
    private final Path storageFolder = Paths.get("src/main/webapp/ImgAudio/");
    //constructor
    ...
    private boolean isAudioFile(MultipartFile file) {
        //Let install FileNameUtils
        String fileExtension = FilenameUtils.getExtension(file.getOriginalFilename());
        return Arrays.asList(new String[] {"mp3","wma","wav"})
                .contains(fileExtension.trim().toLowerCase());
    }
        ...
        try {
            ...
            if (file.isEmpty() ==false) {
                if(isAudioFile(file)){
                    ...
                    throw new RuntimeException("You can only upload audio file");
                }
                String fileExtension = FilenameUtils.getExtension(file.getOriginalFilename());
                generatedFileName = UUID.randomUUID().toString().replace("-", "");
                generatedFileName = generatedFileName+"."+fileExtension;
